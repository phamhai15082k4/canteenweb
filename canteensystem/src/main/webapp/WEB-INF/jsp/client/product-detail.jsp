...
<html lang="vi">
    <head>
        ...
        <title>${p.name} - Chi tiết món ăn</title>
        ...
        <div class="container py-5">
            <div class="mb-4">
                <a href="/menu" class="btn-back">
                    <i class="bi bi-arrow-left-circle-fill fs-4 me-2 text-warning"></i> Về trang Thực đơn
                </a>
            </div>
                ...
                <div class="row g-0">
                        ...
                        <div class="product-img-wrapper">
                            ...
                            <c:choose>
                                ...
                                <c:when test="${not empty p.image}">
                                    <img src="data:image/jpeg;base64,${p.image}" class="product-img" alt="${p.name}">
                                </c:when>
                                <c:otherwise>
                                    <i class="bi bi-image text-muted" style="font-size: 6rem; opacity: 0.2;"></i>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 d-flex flex-column justify-content-center">
                        <div class="card-body p-4 p-lg-5">
                            ...
                            <div class="mb-3">
                                <span class="badge bg-light text-dark border px-3 py-2 rounded-pill me-2"><i class="bi bi-tag text-warning me-1"></i> ${p.category.name}</span>
                                <c:if test="${p.isAvailable}">
                                    <span class="badge bg-success bg-opacity-10 text-success border border-success px-3 py-2 rounded-pill"><i class="bi bi-check-circle-fill me-1"></i> Đang phục vụ</span>
                                </c:if>
                                <c:if test="${!p.isAvailable}">
                                    <span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary px-3 py-2 rounded-pill"><i class="bi bi-pause-circle-fill me-1"></i> Tạm hết món</span>
                                </c:if>
                            </div>

                            <h1 class="product-title mb-3">${p.name}</h1>
                            ...
                            <p class="fs-5 text-muted mb-4" style="line-height: 1.8;">
                                ${p.description != null && p.description != '' ? p.description : 'Hương vị tuyệt hảo, nguyên liệu tươi ngon được chế biến theo công thức đặc biệt của bếp trưởng The Canteen.'}
                            </p>
                                ...
                                <form action="/cart/add" method="post">
                                    ...
                                    <div class="row align-items-center g-3">
                                        <div class="col-4 col-md-3">
                                            <input type="number" name="quantity" value="1" min="1" max="20" class="form-control quantity-input" title="Số lượng">
                                        </div>
                                        <div class="col-8 col-md-9">
                                            <c:choose>
                                                <c:when test="${pageContext.request.userPrincipal == null}">
                                                    <button type="button" class="btn btn-add-cart w-100" data-bs-toggle="modal" data-bs-target="#loginModal">
                                                        <i class="bi bi-cart-plus-fill fs-5 me-2"></i> THÊM VÀO GIỎ
                                                    </button>
                                                </c:when>
                                                <c:otherwise>
                            ...
                            <c:if test="${!p.isAvailable}">
                                <div class="alert alert-warning text-center rounded-4 border-warning border-opacity-25 mt-3 py-3">
                                    <i class="bi bi-emoji-frown fs-4 d-block mb-2 text-warning"></i> 
                                    <strong>Rất tiếc!</strong> Món này hiện đang tạm hết, nhà bếp đang chuẩn bị thêm nguyên liệu. Bạn vui lòng chọn món khác nhé!
                                </div>
                            </c:if>

                            <div class="mt-4 pt-3 d-flex flex-wrap gap-4 small text-muted fw-bold">
                                <div><i class="bi bi-shield-check text-success fs-5 align-middle me-1"></i> An toàn vệ sinh</div>
                                <div><i class="bi bi-clock-history text-primary fs-5 align-middle me-1"></i> Nhanh chóng</div>
                            </div>
                        ...
                        <form action="/perform_login" method="post">
                            ...
                            <div class="text-center mt-4 pt-3 border-top">
                                <p class="mb-0 text-muted">Chưa có tài khoản hệ thống? 
                                    <a href="/register" class="fw-bold text-danger text-decoration-none">Đăng ký tại đây</a>
                                </p>
                            </div>
                        </form>
                    </div>
